<?php
	...
	echo "<br> - echo $this_lid;";
		...
		while($principal_33_row = mysql_fetch_array($principal_33_result)){
			...
			$interests_row = generateInterestsFromPrincipals($this_lid, $principal_33_row[p_from],  $principal_33_row[p_to]);
			...
			$le_row_from = getFirstRow("select * from lawful_employees where le_id = '".$p_from."'");
			$le_row_to = getFirstRow("select * from lawful_employees where le_id = '".$p_to."'");
			...
			echo " เงินต้น  " . $principal_33_row[p_amount] . " บาท </b>";
			
			
			
			
			$total_33_principal += $principal_33_row[p_amount];
		...
		echo "<br> - รวมเงินต้นทั้งหมด = " . $total_33_principal;
		echo "<br> - รวมเงินดอกเบี้ยทั้งหมด = " . $total_33_pending_interests;
