...
if($_POST["do_add_curator"]){
	...
	//yoes 20181107
	//--also add curator_chain here
	if($_POST['le_35_parent'] || 1==1 ){ //1==1 because dropdown is default to 0 anyway
		
		$le_35_parent = $_POST['le_35_parent']*1;
		
		//echo $le_35_parent; exit();
	...
	//yoes 20190104
	//handle extra m35 for new law here
	$curator_selected_year = getFirstItem("select year from lawfulness where LID = '$curator_lid'");
	if($_POST['is_extra_35'] && $curator_selected_year >= 2018 ){ //1==1 because dropdown is defaul 0 anyway
		
		$is_extra_35 = $_POST['is_extra_35']*1;
		
		
	}elseif( $curator_selected_year >= 2018 ){ //1==1 because dropdown is defaul 0 anyway
		
		$is_extra_35 = 0;
		
	}
	
	if($curator_selected_year >= 2018 ){
		//update all parents to extra 35
		$my_parent = $inserted_curator_id;		
		...
		while($my_parent){
			...
			mysql_query($meta_sql);		
			$my_parent = getParentOfCurator($my_parent);
