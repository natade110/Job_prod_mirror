<?php
	...
	$req->password = $password;
	$req->person_code = $the_id;
	$ws = new SoapClient("http://161.82.250.36/ws/services.php?func=02",array("trace" => 1, "exception" => 0));
	...
	if($result->return_code == 0){
		...
		if($is_test_id) {
			// For test ID, create array of children including original and test data
			$children = array(
				array(
					'curator_name' => $child->prefix_name_abbr . ' ' . $child->first_name_thai . ' ' . $child->last_name_thai,
					'curator_idcard' => $child->person_code,
					'curator_gender' => strtolower($child->sex_code),
					'curator_age' => sqlBirthday($child->birth_date),
					'curator_lid' => $curator_lid,
					'curator_parent' => $curator_last_id,
					'curator_disable_desc' => $DEFORM_ID_2_NAME[$child->deform_id],
					'curator_is_disable' => '1'
				),
				array(
					'curator_name' => 'นาย ทดสอบ หนึ่ง',
					'curator_idcard' => '3260100312074',
					'curator_gender' => 'm',
					'curator_age' => '35',
					'curator_lid' => $curator_lid,
					'curator_parent' => '0',
					'curator_disable_desc' => 'ความพิการทางการเห็น',
					'curator_is_disable' => '1'
				),
				array(
					'curator_name' => 'นางสาว ทดสอบ สอง',
					'curator_idcard' => '3260100312074',
					'curator_gender' => 'f',
					'curator_age' => '28',
					'curator_lid' => $curator_lid,
					'curator_parent' => '0',
					'curator_disable_desc' => 'ความพิการทางการได้ยินหรือสื่อความหมาย',
					'curator_is_disable' => '1'
				)
			);

			$the_output .= ", 'children' : " . json_encode($children);
		}
