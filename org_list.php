...
ไม่มีคนรับเอกสาร</span></td><script>						
    ...
    <option value="2" <?php if($_POST["LawfulFlag"] == "2"){echo "selected='selected'";}?>>ปฏิบัติตามกฎหมายแต่ไม่ครบตามอัตราส่วน</option>
    <option value="3" <?php if($_POST["LawfulFlag"] == "3"){echo "selected='selected'";}?>>ไม่เข้าข่ายจำนวน<?php echo $the_employees_word;?></option>
	<option value="5" <?php if($_POST["LawfulFlag"] == "5"){echo "selected='selected'";}?>>นับว่าปฏิบัติตามกฎหมายเนื่องจากเข้าข่ายของคำวินิจฉัยกฤษฎีกา</option>
	<?php //bank add new status = 6 20221227 ?>
	<option value="6" <?php if($_POST["LawfulFlag"] == "6"){echo "selected='selected'";}?>>นับว่าปฏิบัติตามกฎหมายเนื่องจากการยุติการดำเนินคดีทางกฎหมาย</option>
</select></td>
...
ไม่มีคนรับเอกสาร</span></td><script>						
	...
	doToggleNoRecipientFlag();							
					  ...
					  <?php //bank add 20221226 new dropdown law status ?>
                        <?php if($sess_accesslevel == "6" || $sess_accesslevel == "7"){?>

                        <?php }else{ ?>
					  <tr>
                    	  <td bgcolor="#efefef">สถานะงานคดี:</td>
                    	  <td >
								<?php include "ddl_law_status.php"; ?>
						  </td>
                    	  <td colspan="2" >&nbsp;</td>
                  	  </tr>
                        <?php }?>
					  ...
					  <?php  //bank add 20221226 law status filter
					    
						if($_POST["law_status_filter"] || $_POST["law_status_filter"] == 0 && $_POST["law_status_filter"] != "")
						{
								if(isset($_POST['ddl_year']) && $_POST['ddl_year'] != 9999){
									
									$cur_company_year = $_POST['ddl_year'];
									
									$law_array = array('case_type' => "hire",
												   'case_status' => doCleanInput($_POST["law_status_filter"])*1,
												   'case_year' => $cur_company_year);
								...
								$url = "http://203.154.94.105/law_system/law_ws/getCaseList.php?";  
								
								//echo print_r($law_array);
								
								$ch = curl_init();

					
								curl_setopt($ch, CURLOPT_URL, $url . http_build_query($law_array));
								...
								$count = json_encode($array["data"]["Count"]);
								$data = json_encode($array["data"]["CaseListDetail"]);
								...
								$i = 0;
								//$xxx = 0;
								$filter_only_sent_court = "and y.lid in (select meta_lid from lawfulness_meta t where t.meta_lid = y.lid and t.meta_for like '%courted_flag%')";
								$filter_law = " and concat(y.cid, y.year) in (999999";
								for ($i = 0; $i <= $count; $i++)  {
									
									$case_ref_pid = json_encode($array["data"]["CaseListDetail"][$i]["case_ref_pid"]);
									$case_year = json_encode($array["data"]["CaseListDetail"][$i]["case_year"]);
									// Access elements of the array here
									$filter_law .= ",concat($case_ref_pid,$case_year)";
					...
					for($i = 0; $i < count($input_fields); $i++){
						...
						if(strlen($_POST[$input_fields[$i]])>0){
							...
							if($input_fields[$i] == "Province"  ){
								$condition_sql .= " and z.$input_fields[$i] like '".doCleanInput(trim($_POST[$input_fields[$i]]))."'";
							}elseif($input_fields[$i] == "Employees" ){
								$condition_sql .= " and y.$input_fields[$i] >= '".doCleanInput(trim($_POST[$input_fields[$i]]))."'";
							}else{
								$condition_sql .= " and z.$input_fields[$i] like '%".doCleanInput(trim($_POST[$input_fields[$i]]))."%'";
							}
					...
					if(strlen($_POST["CompanyNameThai"]) > 0){
						...
						for($i=0; $i<count($name_exploded_array);$i++){
							...
							if(strlen(trim($name_exploded_array[$i]))>0){
								...
								//yoes 20230108 - company name conditon
								$name_condition_sql .= " and z.CompanyNameThai like '%".doCleanInput($name_exploded_array[$i])."%'";
						...
						//yoes 20230108
						//check จากชื่อเก่าด้วย
						$condition_sql .= "
												or
													
													z.cid in(
														SELECT distinct(cid)
														FROM company_full_log z
														WHERE 1=1
														$name_condition_sql
						...
						$log_name_result = mysql_query($log_name_sql);
						
						//
						$log_name_array = array();
						
						while($log_name_row = mysql_fetch_array($log_name_result)){
							
							$log_name_array[$log_name_row[cid]] .= ",".$log_name_row[companyNameThai];
							//print_r($log_name_row);
							
						}
						
						//print_r($log_name_array);
                          ...
                          <?php if(!$do_hide_company_list && $sess_accesslevel != 4){ ?>
                          	...
                          	<table border="0" style="color: #006699">
                            	<tr>
									...
									<?php //bank add 20230103 new law status 6 ?>
									<td >
                  ...
                  <?php  if(!$do_hide_company_list){ ?>
               	  <table border="1" width="100%" cellspacing="0" cellpadding="5" style="border-collapse:collapse; <?php if($sess_accesslevel == 4){?>display:none;<?php }?>">
                    	<tr bgcolor="#9C9A9C" align="center" >
							...
							<?php //bank add 20222612 law status ?>
							<td>
                           	<div align="center" id="law_status_jquery"><span class="column_header">สถานะงานคดี</span>                       	        
							</div></td>
                          ...
                          <td>
                            ...
                            <td>
								...
								<?php 
									...
									//yoes 20230108 -- ให้ search ได้จากชื่อเก่า	
									//$log_name_array[$log_name_row[cid]] .= ", ".$log_name_row[companyNameThai];
									if($log_name_array[$post_row["CID"]]){
										
										//ต้องเอาชื่อเก่า ที่เท่ากับชื่อปัจุบันออกด้วย
										$old_names = $log_name_array[$post_row["CID"]];
										$old_names = trim(str_replace(",".$post_row["CompanyNameThai"],"",$old_names));
										
										if($old_names){
											echo "<font color=blue style='font-size: 12px;'><br>(ชื่อเก่า - ".substr($old_names,1,strlen($old_names)-1).")</font>";
										}
						...
						<td>
								...
								<?php $chk_sent_court = "select 
														...
														where 
														  a.cid = '".doCleanOutput($post_row["CID"])."'
														  and a.year = '".doCleanOutput($post_row["Year"])."'
														  and b.meta_for like '%courted_flag%'
														";
								$chk_lid = getFirstItem($chk_sent_court);
								
								if($chk_lid == $post_row["Year"]){?>	
						
						
	
							<div align="center" id="law_status_output_<?php echo $total_records ?>">
							

								
								
								วันที่รับรื่อง :  <span id="lawful_case_accepted_date_<?php echo $total_records ?>">  </span>
								<br>
								สถานะปัจจุบัน : <span id="lawful_lov_label_<?php echo $total_records ?>">  </span>
						...
						<script>
						<?php // bank add JQuery 20222712 ?>
						function getLawStatus(cid,year,total_records)	{
						  $.ajax({
							el: '#law_status_output_' + total_records,
							type: 'GET',
							...
							success: function(response) {
							  ...
							  $("#lawful_case_accepted_date_" + total_records).html(nn.case_accepted_date);
							  $("#lawful_lov_label_" + total_records).html(mm[0].lov_label);
						...
						getLawStatus(<?php echo $post_row["CID"]?>,<?php echo $post_row["Year"]?>,<?php echo $total_records?>);
