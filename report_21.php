...
<div align="center">
            <strong>รายละเอียดสถานประกอบการที่ไม่ปฏิบัติตามกฎหมาย แต่มีข้อมูล ม.33,34,35 <?php echo $province_text;?> ประจำปี <?php echo $the_year_to_use;?></strong>
              <br>
</div>
    ...
    <table border="1" align="center" cellpadding="5" cellspacing="0" <?php if(!$is_pdf){?>style="font-size:14px;"<?php }else{?>style="font-size:28px;"<?php }?>>
   	  <thead>
      <tr>
        <td colspan="9"><div align="center"><strong><?php echo $business_type;?></strong></div></td>
      </tr>
      <tr >
        <td width="50" rowspan="2" valign="top"><br /><div align="center" style="vertical-align:middle;"><strong>ลำดับที่ </strong></div></td>
        <td rowspan="2" valign="middle"><div align="center" style="vertical-align:middle;"><strong>เลขทะเบียนนายจ้าง</strong></div></td>
...
(บาท)</strong></div></td>
					...
					where
						...
						and(
							...
							lawfulness.CID in (
								...
								select le_cid from lawful_employees where le_year = '$the_year'
							
							)
							or
							lawfulness.LID in (
							
								select curator_lid from curator where curator_parent = '0'
	...
	while ($lawful_row = mysql_fetch_array($lawful_result)) {
		...
		//
		$this_lawful_employees = getFirstItem("select count(*) from lawful_employees where le_cid = '".$lawful_row["the_company_id"]."' and le_year = '$the_year'");
		
		array_push($employee_array, $this_lawful_employees);
