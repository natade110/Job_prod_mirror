<?php
	...
	if($_POST["LID"]){
		...
		if(!$this_cid){
			$this_cid = getFirstItem("select cid from lawfulness where lid = '".$this_id."'");
		}
        ...
        // Handle different employee types based on access level
        if($sess_accesslevel == "6" || $sess_accesslevel == "7") {
            // Calculate total for government organizations including all employee types
            $update_employees = deleteCommas($_POST["update_employees"])*1; // จำนวนข้าราชการ
            $employees_gov_02 = deleteCommas($_POST["update_employees_gov_02"])*1; // พนักงานราชการ
            $employees_gov_03 = deleteCommas($_POST["update_employees_gov_03"])*1; // ลูกจ้างประจำ
            $employees_gov_04 = deleteCommas($_POST["update_employees_gov_04"])*1; // ผู้ปฏิบัติการที่เรียกชื่ออย่างอื่น

            $this_employess = $update_employees + $employees_gov_02 + $employees_gov_03 + $employees_gov_04;

            // Store individual government employee type counts
            $meta_fields = array(
                'employees_gov_02',
                'employees_gov_03',
                'employees_gov_04'
            ...
            foreach($meta_fields as $field) {
                $meta_sql = "REPLACE INTO lawfulness_meta 
                            (meta_lid, meta_for, meta_value)
                            VALUES (
                                '".$this_id."',
                                '".$field."',
                                '".deleteCommas($_POST["update_".$field])."'
                            )";
