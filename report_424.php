...
if($_POST["cour_sent_format"] == "0"){
	...
	$courted_table_sql = "left join lawfulness_meta t on b.lid = t.meta_lid";
	$courted_meta_sql = "and t.meta_lid not in (select tt.meta_lid from lawfulness_meta tt where tt.meta_lid = b.lid and tt.meta_for = 'courted_flag') ";
}



	
//all type , only no sent = show all year
if($_POST["type_format"] == "All"){
	
	$all_year = 1;
	
	
	$ex1 = ',(case when b.year = ' . $expire2Year . ' then 1
			  else 0 end) expire2Year';
			  
	$ex2 = ',(case when b.year = '. $expire1Year .' then 1
			  else 0 end) expire1Year';
			  
	$ex3 = ',(case when b.year = '. $expire8Month .' then 1
			  else 0 end) expire8Month';
	
	//$courted_table_sql = "left join lawfulness_meta t on b.lid = t.meta_lid";
	//$courted_meta_sql = "and t.meta_lid not in (select tt.meta_lid from lawfulness_meta tt where tt.meta_lid = b.lid and tt.meta_for = 'courted_flag') ";
	$showAll = "AND b.Year in ($expire2Year,$expire1Year,$expire8Month)";
	
//select expire in 2 year && all sent
}elseif($_POST["type_format"] == "Ex_2_year"){
	
	
		$ex1 = ',1 as expire2Year';
		$year_2diff = $now - 8;
		//$expire2Year = $year_2diff + 2;
		$expire2Year_condition = "AND b.Year = $year_2diff";

//select expire in 1 year && only sent
}elseif($_POST["type_format"] == "Ex_1_year"){
	
		$ex2 = ',1 as expire1Year';
		$year_1diff = $now - 9;
		//$expire2Year = $year_2diff + 2;
		$expire1Year_condition = "AND b.Year = $year_1diff";

//select expire in 8 moth && only no sent	
}elseif($_POST["type_format"] == "Ex_8_month"){
	
		$ex3 = ',1 as expire8Month';
		$year_8_month_diff = $now - 9;
		//$expire2Year = $year_2diff + 2;
		$expire8Month_condition = "AND b.Year = $year_8_month_diff";
}
...
<div align="center">
	<strong>รายงานสถานประกอบการที่เข้าข่ายต้องถูกส่งดำเนินคดี เนื่องจากคดีใกล้หมดอายุ</strong>
	<br />
	<br />
</div>
...
<table border="1" align="center" cellpadding="5" cellspacing="0" <?php if(!$is_pdf){?>style="font-size:14px;"<?php }else{?>style="font-size:18px;"<?php }?>>
      ...
      <tr>
        ...
        <td width="<?php echo $w75;?>" rowspan="2" align="center" valign="bottom" ><div align="center"><strong>หมดอายุความใน 8 เดือน</strong></div></td>
		<td width="<?php echo $w75;?>" rowspan="2" align="center" valign="bottom" ><div align="center"><strong>หมดอายุความใน 1 ปี</strong></div></td>
		<td width="<?php echo $w75;?>" rowspan="2" align="center" valign="bottom" ><div align="center"><strong>หมดอายุความใน 2 ปี</strong></div></td>
	  </tr>
      <tr >
...
<?php
				...
				*/
				...
				<td width="<?php echo $w50?>"  valign="top"><div align="center"><?php if($lawful_row["expire8Month"]){echo 'X';}?></div></td> 
				<td width="<?php echo $w50?>"  valign="top"><div align="center"><?php if($lawful_row["expire1Year"]){echo 'X';}?></div></td>	
				<td width="<?php echo $w50?>"  valign="top"><div align="center"><?php if($lawful_row["expire2Year"]){echo 'X';}?></div></td>		
