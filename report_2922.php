...
$querySQL = "
		...
		where
			...
			a.Response like '%<respcode>0</respcode>%'
			
			and
			b.NEPFundCancelDate is not null
			and
			b.NEPFundCancelDate != ''
...
?>
	<div align="center">
		<strong>รายงานการทำ Reverse Transaction ระบบธนาคารกรุงไทย online</strong>
		<br />
	</div>
	<table border="1" align="center" cellpadding="5" cellspacing="0" style="border-collapse: collapse;font-size:<?php echo !$is_pdf ? 14:28; ?>px">
		<thead>
			<tr bgcolor="#9C9A9C" align="center">
				...
				<td>ดอกเบี้ย</td>
				<td>รวม</td>
				<td>วันที่ระบบใบเสร็จยกเลิกรายการ</td>
			</tr>
		</thead>
		<tbody>
		<?php
		...
		?>
			<tr bgcolor="#ffffff" align="center">
				...
				<td><div align="right"><?php echo formatYear($row["Year"]);?></div></td>
				<td><div align="right"><?php echo formatNumber($row["TransactionPrincipalAmount"]) ;?></div></td>
				<td><div align="right"><?php echo formatNumber($row["TransactionInterestAmount"]);?></div></td>
				<td><div align="right"><?php echo formatNumber($row["TransactionTotalAmount"]);?></div></td>
				<td><?php echo formatDateThai($row["NEPFundCancelDate"],1,0);
					//echo $row["LogTime"];?></td>
			</tr>
