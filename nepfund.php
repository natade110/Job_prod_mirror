<?php 
require "nepfund_webservice_message.php";
require "nepfund_webservice_config.php";
...
class CalculatedPayment{
	 ...
	 */
	public function getOutstandingDebts($companyCode, array $branchCodes, array $years, $isFilterLawfulStatus){
		$companySql = "";
	 ...
	 */
	private function getOutstandingPayment($lawfulnessYear, $startInterestDate, $receiptDate, $outstandingAmount, $outstandingInterest, $paidAmount){
		$interest = 0;
// 		print_r("<br />");
...
// 						error_log(var_export($outstandingDebt, true));
							...
							if (($principleAmount == $outstandingDebt->PrincipleAmount) &&
								...
								$lawfulStatus = ($receipt->PaidTotalAmount >= $receipt->TotalAmount)? 1 : 2; /* 1 = ปฏิบัติตามกฎหมาย, 2 = ปฏิบัติตามกฏหมายแต่ไม่ครบตามอัตราส่วน */
		 						$lawfulnessUpdate = "
		 							UPDATE lawfulness l 
				 						INNER JOIN (
				 								SELECT l.lid FROM lawfulness l
				 								inner join company c on l.cid = c.cid
				 								where l.year = $receiptYear AND c.companycode = '$companyCode' and c.branchcode = '$branchCode'
				 						) law ON l.LID = law.LID
			 						SET
			 						LawfulStatus = $lawfulStatus,
			 						pay_status = 1";
...
if ($_GET["test"] && $_SERVER['REQUEST_METHOD'] == 'GET'){
	echo "Version: 1.20160113\r\n<br/>";
	$service = new NepFundService();
	$result = $service->TestConnection();
	echo "DB Connection: ".($result->IsSuccess?"Success":"Fail")."\r\n<br/>";
	if (!$result->IsSuccess){
		echo "Message: $result->ErrorMessage \r\n<br/>";
	}
}else{
	//service
	$server = new SoapServer('NepFund.wsdl', 
			array(
					'soap_version' => SOAP_1_2,
					'features' => SOAP_SINGLE_ELEMENT_ARRAYS					
			));
	$server->setClass("NepFundService");
