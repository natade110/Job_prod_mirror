...
<td valign="top">
  ...
  <form method="post" action="payment_calculate_hire.php?a=<?php echo rand(0,999);?>">
...
<script>
	...
	function getTodayInterests(start_money, interest_days, interest_rate){
		...
		//alert(toFixed(result_interest,2));
		result_interest = toFixed(result_interest,3);
		result_interest = toFixed(result_interest,2);
		...
		//alert(Math.round(2511.825 * 100)/100);
		
		/*var num = Number(result_interest) // The Number() only visualizes the type and is not needed
	...
	function check_ddl_values(what){
		...
		if(this_day > 0 && this_month >= 0 && this_year > 0){
			...
			document.getElementById(what + "_interest_money_to_show").innerHTML = this_interest_money;
	...
	function reCalculateValue(){
		...
		<?php }else{?>
			...
			wage_rate = <?php echo getFirstItem("select var_value from vars where var_name = 'wage_$the_year' ") ;?>;
		
		<?php }?>
		
		
		//alert(<?php echo $the_year;?> + " - " + wage_rate);
