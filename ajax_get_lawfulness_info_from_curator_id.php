<?php
	...
	if(!$the_curator_id){
		$the_curator_id = $_GET["the_curator_id"]*1;
	}
	...
	if($the_mode == "ejob"){		
		...
		//yoes 20231004 --
		$curator_row = getFirstRow("select * from curator$es_table_name_suffix where curator_id = '$the_curator_id'");
		$this_lid = $curator_row["curator_lid"];	
		
		$ejob_special_wh = " and p_lid = '$this_lid'";
	...
	$principal_sql = "
		...
		select
			*
		from
			lawful_35_principals$es_principle_table_suffix
		where
			(
				p_from = '".$p_from."'
				or
	...
	//common vars
	$curator_row = getFirstRow("select * from curator$es_table_name_suffix where curator_id = '$the_curator_id'");
	$lawfulness_row = getFirstRow("select * from lawfulness$es_table_name_suffix where lid = '".$curator_row["curator_lid"]."'");	
	...
	while($principal_row = mysql_fetch_array($principal_result)){
		...
		if($principal_row && $this_lawful_year >= 2018){
			...
			$interests_row = generateInterestsFromPrincipals($this_lid, $principal_row[p_from],  $principal_row[p_to], "m35", "$es_principle_table_suffix", date("Y-m-d"), $the_mode);
